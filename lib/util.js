...
function _registerGlobalTokens(weatherData) {
    ...
    Homey.manager('flow').registerToken("alert_description", {
        type: 'string',
        title: __("weather.alert_description")
    }, function(err, token) {
        if (err) return Homey.log('alert_description registerToken error:' + JSON.stringify(err));

        token.setValue(weatherData.alert_description, function(err) {
            if (err) return Homey.log('alert_description setValue error:' + JSON.stringify(err));
        });
        tokens.push(token);
